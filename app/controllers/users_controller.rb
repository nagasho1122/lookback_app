class UsersController < ApplicationController
  ...
  def show
    ...
    @reviewsNotNear = @user.reviews.where("deadline_at > ?", after_three_days).limit(3)
    #復習リストの期限切れじゃないかつ直近三日のものを最大で６個取得
    @reviewsNear = @user.reviews.where(deadline_at:  DateTime.now..after_three_days).limit(6)
    #復習リストの期限切れのものを最大で3個取得
    @reviewsExpire = @user.reviews.where("deadline_at < ?", today).limit(3)
    #復習リストの科目をリストで取得
    @subjects = []
